		...
		if ( flag === "boolean" ){
			...
			$.ajax ({
					...
					success: function( data ){
						if(!data.success){
							render.feedbackModal("Invalid Login", data.message, "Try Again" );
						} else {
							USER_NAME = data.name;
  ...
  function publicRegisterUser() {
	...
	function publicRegisterNewUser( elem ) {
			...
			$.ajax ({
			    ...
			    success: function(data){
						if(!data.success){
							render.feedbackModal("Register Error", data.message, "Try Again" );
						} else {
							$('.register').empty();
							render.feedbackModal("Success", data.message, "Login Now" );
							render.loginView();
						}
			    },
					error: function () {
 ...
 function publicUserVoteCheck( surveyId, voteOption ) {
  ...
  $.ajax ({
 		  ...
 		  data: JSON.stringify(reqUsr),
 			...
 			error: function () {
					console.log("failed");
					render.feedbackModal("Server Error", "Please try and vote again.", "Try Again" );
 			}
 		});
 }
